...
namespace JuliusSweetland.OptiKey.UI.Views.Keyboards.Common
{
    ...
    public partial class CustomKeyboard : KeyboardView
    {
        ...
        private Key CreateKeyWithBasicProps(XmlKey xmlKey)
        {
            ...
            Key newKey = new Key();
            newKey.Text = xmlKey.Text.ToStringWithValidNewlines();
            ...
            bool hasString = xmlKey.Text.Length > 0;
        ...
        void AddLinkKey(XmlLinkKey xmlKey) {
            ...
            if (null != xmlKey.Link)
            {
                ...
                newKey.Value = new KeyValueLink(Path.Combine(rootDir, xmlKey.Link), xmlKey.Replace);
            }
            else
            {
                Log.ErrorFormat("No link found for link key with label {0}", xmlKey.Text);
            }
        ...
        void AddTextKey(XmlTextKey xmlKey)
        {
            ...
            if (null != xmlKey.Value)
            {
                newKey.Value = new KeyValue(xmlKey.Value);
            }
            else
            {
                Log.ErrorFormat("No value found in text key with label {0}", xmlKey.Text);
            }
        ...
        void AddBehaviourKey(XmlBehaviourKey xmlKey)
        {
            ...
            if (xmlKey.FunctionKey.HasValue)
            {
                ...
                Log.ErrorFormat("No FunctionKey found for key with label {0}", xmlKey.Text);
            }
